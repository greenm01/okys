...
test "sparse compute descriptors use storage resources and generated slots" {
    ...
    const segment_indices = sokol_device.View{ .id = 9 };
    ...
    const fine_bindings = sokol_device.sparseFineBindings(calls, segments, clips, tasks, clip_indices, segment_indices, surface, image, sampler);
    ...
    try testing.expectEqual(segment_indices, fine_bindings.views[sokol_device.sparse_segment_indices_view_slot]);
